// FICHIER: ./components/Statistics/METEvolutionChart.js
...
import { TrendingDown, BarChart3, XCircle } from 'lucide-react-native';

const screenWidth = Dimensions.get('window').width;

export default function METEvolutionChart({ 
  data, 
  isLoading = false,
  period = 'month' 
}) {
  ...
  useEffect(() => {
    console.log('=== METEvolutionChart DEBUG ===');
    console.log('Props reçues:', { data, isLoading, period });
    
    if (data) {
      console.log('Data structure:', {
        hasLabels: !!data.labels,
        labelsLength: data.labels?.length,
        hasMetData: !!data.metData,
        metDataCompleted: data.metData?.completed,
        metDataCompletedLength: data.metData?.completed?.length,
        metDataRate: data.metData?.rate,
        entireDataObject: data
      });
    }
  ...
  const getFilteredLabelsAndData = () => {
    console.log('[METChart] getFilteredLabelsAndData appelée');
    
    if (!data || !data.labels || data.labels.length === 0) {
      console.log('[METChart] Pas de données ou pas de labels, utilisation des données par défaut');
      return {
        labels: ['...'],
        data: [0]
      };
    ...
    const originalData = data.metData.completed;
    
    console.log('[METChart] Données originales:', {
      originalLabels,
      originalData,
      originalDataSum: originalData.reduce((a, b) => a + b, 0)
    });
    ...
    console.log('[METChart] Données filtrées:', {
      filteredLabels,
      filteredData,
      filteredDataSum: filteredData.reduce((a, b) => a + b, 0)
    });
  ...
  const getChartData = () => {
    console.log('[METChart] getChartData appelée');
    ...
    console.log('[METChart] Données finales pour le graphique:', result);
    return result;
  };

  // Métriques d'en-tête simplifiées
  const getHeaderMetrics = () => {
    console.log('[METChart] getHeaderMetrics appelée');
    
    if (!data || !data.metData) {
      console.log('[METChart] Pas de données metData');
      return { total: 0, average: 0 };
    }

    const values = data.metData.completed;
    console.log('[METChart] Valeurs pour métriques:', values);

    if (values.length === 0) {
      console.log('[METChart] Tableau des valeurs vide');
      return { total: 0, average: 0 };
    }
    ...
    console.log('[METChart] Métriques calculées:', { total, average, valuesLength: values.length });
  ...
  if (isLoading) {
    console.log('[METChart] Mode loading actif');
    return (
      <View style={styles.container}>
        <View style={styles.loadingContainer}>
          <Text style={styles.loadingText}>Chargement du graphique MET...</Text>
        </View>
      </View>
    );
  }

  console.log('[METChart] Rendu du composant avec métriques:', metrics);
      ...
      <View style={styles.header}>
        <View style={styles.titleSection}>
          ...
          <View style={styles.titleContainer}>
            <Text style={styles.title}>MET évitées</Text>
      ...
      <View style={styles.legend}>
        <View style={styles.legendItem}>
          ...
          <Text style={styles.legendText}>
            Nombre de MET évitées par jour
          </Text>
        </View>
      </View>
    </View>
